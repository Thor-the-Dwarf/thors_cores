...
class NewLevelScreen extends StatefulWidget {
  ...
  const NewLevelScreen({super.key, required this.unlocked});

  @override
  _NewLevelScreenState createState() => _NewLevelScreenState();
}

class _NewLevelScreenState extends State<NewLevelScreen> with TickerProviderStateMixin {
  bool isLoading = true;
  List<TreeNode> tree = [];
  ...
  bool _isNodeUnlocked(TreeNode node) {
    // Prüfe, ob der Knoten selbst in der unlocked-Liste ist
    if (widget.unlocked.contains(node.id)) {
      return true;
    }

    // Finde den Elternknoten rekursiv, indem wir den Baum durchsuchen
    TreeNode? findParent(TreeNode target, List<TreeNode> nodes) {
      for (var parent in nodes) {
        if (parent.children.contains(target)) {
          return parent;
        }
        if (parent.children.isNotEmpty) {
          final found = findParent(target, parent.children);
    ...
    // Prüfe, ob ein Elternknoten freigeschaltet ist
    TreeNode? parent = findParent(node, tree);
    while (parent != null) {
      if (widget.unlocked.contains(parent.id)) {
        return true;
      }
    ...
    return Container(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final isUnlocked = _isNodeUnlocked(node);

          return InkWell(
            onTap: isUnlocked && node.children.isNotEmpty
                ? () => _toggleNode(node)
                : null,
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  GestureDetector(
                    onTap: isUnlocked && node.hasCores && !node.isCore
                        ? () {
                      print(
                    ...
                    child: Icon(
                      ...
                      color: isUnlocked
                          ? (node.hasCores ? Colors.yellow : Colors.lightBlue)
                          : Colors.grey,
                    ),
                  ),
                  const SizedBox(width: 8),
        ...
        child: Stack(
          children: [
            ...
            if (_selectedLevelId != null &&
                _isNodeUnlocked(tree.firstWhere(
                        (node) => node.id == _selectedLevelId,
                    orElse: () => TreeNode(id: '', name: ''))))
              Container(
                color: Colors.transparent,
